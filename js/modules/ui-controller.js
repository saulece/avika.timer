...
Avika.ui = {
    ...
    generateCategoryButtons: function() {
        const categories = Avika.config.categoryNames || {};
        let buttonsHTML = '';
        for (const categoryId in categories) {
            if (Object.hasOwnProperty.call(categories, categoryId)) {
                const categoryName = categories[categoryId];
                buttonsHTML += `<button class="category-btn" data-category="${categoryId}">${categoryName}</button>`;
            }
        }
        return buttonsHTML;
    },
    ...
    // ANTES de cualquier manipulación, detener todos los cronómetros activos para evitar fugas de memoria
    deliveryBody.querySelectorAll('tr[data-ready-timer-id]').forEach(row => {
        const timerId = parseInt(row.dataset.readyTimerId, 10);
        if (timerId) {
    ...
    createDeliveryRow: function(order) {
        ...
        } else if (order.readyTimestamp) {
            // Si está listo pero aún no ha salido, mostrar tiempo desde que está listo
            timerCell.className = 'ready-timer';
            timerCell.setAttribute('data-ready-timestamp', order.readyTimestamp);

            // Iniciar un cronómetro para el tiempo de espera en mostrador
            const readyTime = new Date(order.readyTimestamp).getTime();
            const timerId = setInterval(() => {
                const now = new Date().getTime();
                const elapsedSeconds = Math.floor((now - readyTime) / 1000);
                timerCell.textContent = Avika.utils.formatElapsedTime(elapsedSeconds);
            }, 1000);

            // Guardar el ID del intervalo en la fila para poder limpiarlo después
            row.dataset.readyTimerId = timerId;
        } else {
